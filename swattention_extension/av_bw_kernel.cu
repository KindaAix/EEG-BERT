            ...
            if (z < kernel_size){
                ...
                if ((current_pos >= 0) && (current_pos < seq_len)){
                    ...
                    for (int dimOffset=0; dimOffset < d_output.size(3); ++dimOffset)
                        updt += d_output[b][h][pos][dimOffset] * values[b][h][current_pos][dimOffset];
                }
                d_attn_weight[b][h][pos][z]=updt;
            }
        ...
        if (y < d_values.size(2)){
            ...
            if (z < d_values.size(3)){
                ...
                for (int current_pos=q_start_pos; current_pos<q_end_pos; ++current_pos){
                    --offset;
                    if ((current_pos >= 0) && (current_pos < seq_len)){
                        updt += attn_weight[b][h][current_pos][offset] * d_output[b][h][current_pos][z]; 
                    }            
                }
                d_values[b][h][pos][z]=updt; 
...
std::vector<torch::Tensor> av_bw_cu(
    ...
    TORCH_CHECK(attn_weight.size(2) == values.size(2), "Attention Weights and Value should have same Token Nums");
