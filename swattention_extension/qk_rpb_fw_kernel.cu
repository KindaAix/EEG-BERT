            ...
            if (z < kernel_size){
                ...
                if ((current_pos >= 0) && (current_pos < seq_len)){
                    ...
                    for (int dimOffset=0; dimOffset < queries.size(3); ++dimOffset)
                        updt += queries[b][h][pos][dimOffset] * keys[b][h][current_pos][dimOffset];
                    updt += rpb[h][z];
                }
                else{
                    updt = scalar_t(-INFINITY);
                }
                attn_weight[b][h][pos][z]=updt;
...
torch::Tensor qk_rpb_fw_cu(
    ...
    TORCH_CHECK(rpb.size(1) == attention_span, "Last dim of Relative_Position_Bias should equal Kernel_Size");
