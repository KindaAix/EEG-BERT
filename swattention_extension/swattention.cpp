...
torch::Tensor qk_forward(
    ...
    return qk_fw_cu(queries, keys, seq_len, kernel_size, cuda_threads);
}
...
std::vector<torch::Tensor> qk_backward(
    ...
    return qk_bw_cu(d_attn_weight, queries, keys, seq_len, kernel_size, cuda_threads);
}
...
std::vector<torch::Tensor> qk_rpb_backward(
    ...
    return qk_rpb_bw_cu(d_attn_weight, queries, keys, seq_len, kernel_size, cuda_threads);
}
...
torch::Tensor qk_rpb_forward(
    ...
    return qk_rpb_fw_cu(queries, keys, rpb, seq_len, kernel_size, cuda_threads);
}
...
torch::Tensor av_forward(
    ...
    return av_fw_cu(attn_weight, values, seq_len, kernel_size, cuda_threads);
}
...
std::vector<torch::Tensor> av_backward(
    ...
    return av_bw_cu(d_output, attn_weight, values, seq_len, kernel_size, cuda_threads);
}
